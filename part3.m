clear
[modelT,~,~,~] = stlread(".\models\uvwv2.stl");
preformAngle = 70;
pointsPerFacet = 40;
angleDif = 1;
sectionPoint = [0 0 0];
%%
[sectionPoints,colors] = showSurface(modelT,pointsPerFacet,preformAngle);
givenSurface = pointCloud(sectionPoints,Color=colors);
pcshow(givenSurface)
%%
function [sectionPoints,colors] = showSurface(modelT,pointsPerFacet,preformAngle)
    totalFacets = size(modelT.ConnectivityList,1);
    sectionPoints = ones(pointsPerFacet*totalFacets,3);
    green = [0 255 0]; %default color
    colors =  repmat(green,pointsPerFacet*totalFacets,1);
    facetNormals = faceNormal(modelT,(1:totalFacets)');

    for i = 1:totalFacets
        v = modelT.Points(modelT.ConnectivityList(i,:),:); 
        randPoints = randPointsOnFacet(v,pointsPerFacet);
        sectionPoints((i-1)*pointsPerFacet+1:(i)*pointsPerFacet,:) = randPoints;
        if(acosd(abs(facetNormals(i,3)))>preformAngle)
            disp(acosd(abs(facetNormals(i,3))))
            colors((i-1)*pointsPerFacet+1:i*pointsPerFacet,:) = repmat([255 0 0],pointsPerFacet,1);
        end
    end
